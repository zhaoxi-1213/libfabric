...
```mermaid
flowchart TD
    A[开始: cxip_mr_init(dom, attr, flags, mr)] --> B[初始化自旋锁 ofi_spin_init]
    B --> C[配置 mr_fid.fid 的 fclass/context/ops]
    C --> D[将 mr_fid.key 置为 FI_KEY_NOTAVAIL]
    D --> E[保存域指针、flags 与 attr 快照]
    E --> F[初始化事件字段 count_events/match/access/rma]
    F --> G[提取 IOV[0] 的 buf 与 len]
    G --> H{attr.access 含远程访问?}
    H -- 是 --> I[调用 cxip_domain_ctrl_id_alloc 获取 req_id]
    I --> J{分配成功?}
    J -- 否 --> K[记录告警\n销毁锁\n返回 -FI_ENOSPC]
    J -- 是 --> L[继续初始化]
    H -- 否 --> L
    L --> M[设置 mr_id=-1\nreq.mr.mr=mr\nmem_desc 指向自身]
    M --> N[状态置为 CXIP_MR_DISABLED]
    N --> O[返回 FI_SUCCESS]
```
